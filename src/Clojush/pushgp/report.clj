...
(defn report-and-check-for-success-with-gp-guidance
  "Reports on the specified generation of a pushgp run. Returns the best
   individual of the generation."
  [population generation
   {:keys [error-function report-simplifications
  ...
  (let [err-fn (cond
    ...
    (update-past-average-errors (*' 1.0 (/ (reduce +' (map :total-error sorted)) (count population))))
    
    ;(write-gp-guidance-info generation)
    (when (> generation 2)
      (set-delta-average-errors))
