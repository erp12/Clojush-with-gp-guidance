(ns clojush.pushgp.pushgp
  ...
  (:use [clojush globals util pushstate random individual evaluate]
        ...
        [clojush.gpguidance.error-and-gp-operator-recording]
        [clojush.gpguidance.gp-operator-guiding]))
...
(def push-argmap
  (atom (sorted-map
          ...
          ;;----------------------------------------
          :reproduction-probability      0.25 ;; Direct reproduction, which makes a direct copy of the parent
          :mutation-probability          0.25 ;; Subtree mutation, similar to that found in tree-GP
          :crossover-probability         0.25 ;; Subtree crossover, similar to that found in tree-GP
          :simplification-probability    0.0 ;; Auto-simplification of the program
          :ultra-probability             0.25 ;; Uniform Linear Transformation with Repair and Alternation -- a uniform crossover and mutation operator
          ...
          ;;
          ;;---------------------------
          ;; Eddie's GP final arguments
          ;;---------------------------
          :gp-guidance-learning-rate 0.5
          )))
...
(defn pushgp-with-gp-guidance
  "The top-level routine of pushgp. EDDIE MODIFIED THIS ONE"
  ([] (pushgp '()))
  ([args]
    (reset! timer-atom (System/currentTimeMillis))
    (load-push-argmap args)
    (random/with-rng (random/make-mersennetwister-rng (:random-seed @push-argmap))
      ...
      (let [{:keys [pop-agents child-agents rand-gens random-seeds]} (make-agents-and-rng @push-argmap)]
        ...
        (loop [generation 0]
          ...
          (if (> generation 3)
            (reset! push-argmap (guide-gp-operators @push-argmap))
            (reset! push-argmap (set-rand-operator-probs @push-argmap)))
          (reset! push-argmap (make-sum-one @push-argmap))
          ...
          (update-past-genetic-operator-probs {:reproduction-probability (:reproduction-probability @push-argmap)
                                               :mutation-probability (:mutation-probability @push-argmap)
                                               :crossover-probability (:crossover-probability @push-argmap)
                                               :simplification-probability (:simplification-probability @push-argmap)
                                               :ultra-probability (:ultra-probability @push-argmap)
                                               :gaussian-mutation-probability (:gaussian-mutation-probability @push-argmap)
                                               :boolean-gsxover-probability (:boolean-gsxover-probability @push-argmap)
                                               :deletion-mutation-probability (:deletion-mutation-probability @push-argmap)
                                               :parentheses-addition-mutation-probability (:parentheses-addition-mutation-probability  @push-argmap)})
          
          ;; report and check for success
          (let [outcome (report-and-check-for-success-with-gp-guidance (vec (doall (map deref pop-agents)))
                                                                       generation @push-argmap)]
            (cond (= outcome :failure) (do (printf "\nFAILURE\n") (flush))
                  (= outcome :continue) (do (timer @push-argmap :report)
                                            (println "\nProducing offspring...")
